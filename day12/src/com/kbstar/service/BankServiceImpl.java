...
public class BankServiceImpl implements BankService<UserDTO, AccountDTO, TransactionDTO, String, String> {
	...
	public void makeAccount(String k, double money) throws Exception {
		...
		AccountDTO account = new AccountDTO(accNo, money, k);
	...
	public void transaction(String sendAcc, String receiveAcc, double balance, String desc) throws Exception {
		...
		// 거래 내역 추가
		TransactionDTO trn = new TransactionDTO(MakeAccountNumber.makeTrNum(), sendAcc, balance, "O", desc);
		transactionDao.insert(trn);
		// SMS, email 전송
		UserDTO user = userDao.select(acc.getHolder());
		notification.sendEmail(user.getEmail(), sendAcc + "에서 " + balance + "원이 출금 되었습니다.");
		notification.sendSMS(user.getContact(), sendAcc + "에서 " + balance + "원이 출금 되었습니다.");
